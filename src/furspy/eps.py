...
class eps():
    ...
    def couple2D(self):
        ...
        ``Tb_nu_map.npy`` generated by :func:`gen_freq` does not account for chromatic distortions in the antenna. To use this function users must provide an array, named ``D.npy``, which should be in the shape of :math:`N_{\\mathrm{pix}} \\times N_{\\nu}`. Put this array into the path where you have all the other outputs. An output file will be generated called ``T_ant.npy``.
